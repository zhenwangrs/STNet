...
def imageListTemporalRescale(clip, temp_scaling=0.2, frame_interval=1):
    min_len = 32
    max_len = int(np.ceil(230/frame_interval))
    L = 1.0 - temp_scaling
    U = 1.0 + temp_scaling
...
class RandomErase(object):
    """
    Randomly erase a fixed number of consecutive frames in the video using transforms.RandomErasing
    """
    ...
    def __init__(self):
        self.transform = transforms.Compose([
                ...
                # transforms.ToTensor(),
                # 2. 使用 RandomErasing 进行随机擦除
                transforms.RandomErasing(p=0.5, scale=(0.02, 0.2), ratio=(0.3, 3.3)),
            ])

    def __call__(self, clip):
        # if isinstance(clip[0], np.ndarray):
        #     clip = [PIL.Image.fromarray(img) for img in clip]
        # elif isinstance(clip[0], PIL.Image.Image):
        #     pass
        # else:
        ...
        transformed_clip = []
        for img in clip:
            transformed_clip.append(self.transform(img))
        transformed_clip = torch.stack(transformed_clip)
        return transformed_clip


class RandomRotate(object):
    def __init__(self, degree, ratio=0.5):
        self.transform = transforms.Compose([
...
class RandomAugment(object):
    ...
    def __call__(self, clip):
        ...
        for img in clip:
            if random.random() < self.ratio:
                transformed_clip.append(np.array(self.transform(PIL.Image.fromarray(img))))
            else:
                transformed_clip.append(img)
            # transformed_clip.append(self.transform(img))
        # transformed_clip = torch.stack(transformed_clip)
