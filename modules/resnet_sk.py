...
from modules.sk import SKLayer, AffSkLayer
...
class Get_Correlation(nn.Module):
    ...
    def forward(self, x):
        ...
        s = F.sigmoid(aggregated_x)-0.5
        v = features * (F.sigmoid(aggregated_x)-0.5)
...
class FramewiseLoss(nn.Module):
    ...
    def forward(self, seq):
        b, c, t, h, w = seq.shape
        seq = F.avg_pool3d(seq, (1, h, w)).reshape(b, c, t)
        ...
        y = torch.ones(b * (t-1)).to(seq.device) * -1
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dropout_ration=0.5):
        self.inplanes = 64
...
if __name__ == '__main__':
    t = torch.randn(2, 128, 10, 7, 7)
    frame_loss = FramewiseLoss()
    loss = frame_loss(t)
