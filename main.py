...
class Processor():
    def __init__(self, arg):
        ...
        shutil.copy2('./modules/resnet_sk.py', self.arg.work_dir)
        shutil.copy2('./modules/sk.py', self.arg.work_dir)
        shutil.copy2('./modules/rnnt.py', self.arg.work_dir)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            seq_model_list = []
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch + 1):
                save_model = epoch % self.arg.save_interval == 0
                eval_model = epoch % self.arg.eval_interval == 0
                epoch_time = time.time()
                # train end2end model
                seq_train(self.data_loader['train'], self.model, self.optimizer, self.scheduler, self.device, epoch, self.recoder, self.arg.batch_accum)
                if eval_model:
                    ...
                    if epoch > 20:
                        dev_wer = seq_eval(self.arg, self.data_loader['test'], self.model, self.device,
                                           'test', epoch, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
                        self.recoder.print_log("Test WER: {:05.2f}%".format(dev_wer))
                if save_model:
    ...
    def loading(self):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=self.arg.optimizer_args['base_lr'], weight_decay=self.arg.optimizer_args['weight_decay'])
        # optimizer = torch.optim.ASGD(model.parameters(), lr=self.arg.optimizer_args['base_lr'])
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.arg.num_epoch)

        if self.arg.load_weights:
            self.load_model_weights(model, self.arg.load_weights)
        elif self.arg.load_checkpoints:
            self.load_checkpoint_weights(model, optimizer, scheduler)

        model = self.model_to_device(model)
        # model.register_hooks(model)

        optimizer = self.optimizer_to_device(optimizer)
    ...
    def optimizer_to_device(self, optimizer):
    ...
    def load_model_weights(self, model, weight_path):
        ...
        #     if k not in model.state_dict().keys() or v.shape != model.state_dict()[k].shape:
        #         print('Can Not Load Weights: {}.'.format(k))
        #         weights.pop(k)
    ...
    def load_checkpoint_weights(self, model, optimizer, scheduler):
