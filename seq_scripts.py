...
def seq_train(loader, model, optimizer, scheduler, device, epoch_idx, recoder, batch_accum=1):
    model.train()
    loss_value = []
    clr = [group['lr'] for group in optimizer.param_groups]
    scaler = GradScaler()
    for batch_idx, data in enumerate(tqdm(loader), start=1):
        ...
        with autocast():
            ...
            loss, conv_ctc_loss, seq_ctc_loss, dist_loss = model.criterion_calculation(ret_dict, label, label_lgt)
        ...
        if batch_idx % recoder.log_interval == 0:
            recoder.print_log(
                '\tEpoch: {}, Batch({}/{}) done. Loss: {:.8f}, conv_ctc_loss: {:.8f}, seq_ctc_loss: {:.8f}, dist_loss: {:.8f}, frame_loss: {:.8f}, csl_loss: {:.8f}, lr:{:.6f}'
                    .format(epoch_idx, batch_idx, len(loader), loss.item(), conv_ctc_loss.item(), seq_ctc_loss.item(), dist_loss.item(), ret_dict['frame_loss'], ret_dict['csl_loss'], clr[0]))
        del ret_dict
        del loss
        del conv_ctc_loss
        del seq_ctc_loss
...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir, recoder, evaluate_tool="python"):
    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        label_lgt = device.data_to_device(data[3])
        with torch.no_grad():
            ret_dict = model(vid, vid_lgt, label=label, label_lgt=label_lgt, is_train=False)
    ...
    del label
    del label_lgt
    recoder.print_log(f"Epoch {epoch}, {mode} conv: {conv_ret: 2.2f}%", f"{work_dir}/{mode}.txt")
    recoder.print_log(f"Epoch {epoch}, {mode} lstm: {lstm_ret: 2.2f}%", f"{work_dir}/{mode}.txt")
    return lstm_ret
